...
# Const pattern
`pattern_i` consists with an atomic object with less than one length,
such as numeric, literal, NA, NULL. When x is matched with `pattern_i`, `res_i`
...
# Guard clauses
when `pattern_i` uses `Compare` family (and adding `!`, `any`, `and`,
`identity`, `isTRUE`) as top of node of language object, it is regarded
as guard clauses, and when the result of evaluating `pattern_i` is TRUE,
...
# isTRUE(x > 5) -> FALSE, isTRUE(x > 3) -> FALSE, 
